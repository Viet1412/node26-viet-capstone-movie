...
const { Showtime: Entity } = require("../models");
  ...
  search: async (searchKeyWord, pagination) => {
    ...
    try {
      ...
      const foundEntities = await Entity.findAndCountAll({
        where: {
          [Op.or]: [{ dateTime: { [Op.like]: `%${searchKeyWord}%` } }],
        },
    ...
    try {
      ...
      for (let index = 0; index < dataNewEntities.length; index++) {
        let isExist = await Entity.findOne({
          where: { dateTime: dataNewEntities[index].dateTime },
        });
        if (isExist) {
          throw new AppError(
            400,
            `${entityName} <${dataNewEntities[index].dateTime}> already exists`
          );
        }
        dataNewEntities[index].date =
          dataNewEntities[index].dateTime.split(" ")[0];
      }
